...
const UserPicker = ({ list, selected, onToggle, empty, accentColor = '#6366f1', accentBg = 'rgba(99,102,241,0.1)', t }) => (
  <div style={{ maxHeight:'158px', overflowY:'auto', border:`1px solid ${t.border}`, borderRadius:'10px', background:t.inputBg, padding:'4px' }}>
    {list.length === 0
      ...
      : list.map(u => (
        <label key={u.id} style={{ display:'flex', alignItems:'center', gap:'10px', padding:'8px 10px', borderRadius:'8px', cursor:'pointer', background: selected.includes(u.id) ? accentBg : 'transparent', border:`1px solid ${selected.includes(u.id) ? accentColor : 'transparent'}`, transition:'all 0.15s' }}>
          <input type="checkbox" checked={selected.includes(u.id)} onChange={() => onToggle(u.id)} style={{ accentColor, width:'15px', height:'15px', flexShrink:0 }} />
          <div style={{ width:'28px', height:'28px', borderRadius:'50%', flexShrink:0, background:`hsl(${u.name?.charCodeAt(0)*13%360},65%,50%)`, display:'flex', alignItems:'center', justifyContent:'center', color:'white', fontSize:'11px', fontWeight:'700' }}>
          ...
          <span style={{ padding:'2px 7px', borderRadius:'20px', fontSize:'10px', fontWeight:'600', flexShrink:0, background: u.role==='client' ? 'rgba(52,211,153,0.15)' : u.role==='admin' ? 'rgba(167,139,250,0.15)' : 'rgba(96,165,250,0.15)', color: u.role==='client' ? '#34d399' : u.role==='admin' ? '#a78bfa' : '#60a5fa' }}>{u.role}</span>
        </label>
      ))
    }
  </div>
...
const FormBody = ({ f, setF, ms, setMs, cs, setCs, nonClients, clients, dark, t }) => {
  ...
  const flt = (list, q) => list.filter(u => u.name.toLowerCase().includes(q.toLowerCase()) || u.email.toLowerCase().includes(q.toLowerCase()));
  const toggle = (field, id) => setF(prev => ({ ...prev, [field]: prev[field].includes(id) ? prev[field].filter(x => x !== id) : [...prev[field], id] }));
  const inp = (key, extra = {}) => ({ width:'100%', padding:'11px 14px', borderRadius:'10px', boxSizing:'border-box', border:`1.5px solid ${focused === key ? '#6366f1' : t.border}`, background:t.inputBg, color:t.text, fontSize:'14px', outline:'none', transition:'all 0.2s', fontFamily:"'DM Sans',sans-serif", ...extra });
  return (
    <>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'14px', marginBottom:'14px' }}>
        {[{k:'name',label:'Project Name',ph:'e.g. Website Redesign'},{k:'description',label:'Description',ph:'Brief project description'}].map(fd => (
          <div key={fd.k}>
            ...
            <input placeholder={fd.ph} value={f[fd.k]} onFocus={() => setFocused(fd.k)} onBlur={() => setFocused('')} onChange={e => setF({...f, [fd.k]: e.target.value})} style={inp(fd.k)} />
          </div>
        ))}
      </div>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:'14px', marginBottom:'20px' }}>
        <div>
          ...
          <select value={f.status} onChange={e => setF({...f, status:e.target.value})} onFocus={() => setFocused('status')} onBlur={() => setFocused('')} style={inp('status')}>
        ...
        {[{k:'startDate',label:'Start Date'},{k:'endDate',label:'Deadline'}].map(fd => (
          <div key={fd.k}>
            <label style={lbl}>{fd.label}</label>
            <input type="date" value={f[fd.k]} onFocus={() => setFocused(fd.k)} onBlur={() => setFocused('')} onChange={e => setF({...f, [fd.k]: e.target.value})} style={inp(fd.k, {colorScheme: dark?'dark':'light'})} />
          </div>
        ))}
      </div>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'20px' }}>
        <div>
          ...
          <input placeholder="Search members..." value={ms} onChange={e => setMs(e.target.value)} onFocus={() => setFocused('ms')} onBlur={() => setFocused('')} style={{...inp('ms'), marginBottom:'8px'}} />
          <UserPicker list={flt(nonClients, ms)} selected={f.memberIds} onToggle={id => toggle('memberIds', id)} empty="No members found" t={t} />
        </div>
        <div>
          <label style={lbl}><span style={{color:'#34d399'}}>◉ </span>Assign Clients ({f.clientIds.length} selected)</label>
          <input placeholder="Search clients..." value={cs} onChange={e => setCs(e.target.value)} onFocus={() => setFocused('cs')} onBlur={() => setFocused('')} style={{...inp('cs'), marginBottom:'8px'}} />
          <UserPicker list={flt(clients, cs)} selected={f.clientIds} onToggle={id => toggle('clientIds', id)} empty="No clients found" accentColor="#34d399" accentBg="rgba(52,211,153,0.1)" t={t} />
        </div>
      </div>
    </>
  );
...
export default function Projects() {
  ...
  const openEdit = (p) => {
    setEditingId(p.id);
    setEditForm({ name: p.name||'', description: p.description||'', status: p.status||'active', startDate: p.startDate?p.startDate.slice(0,10):'', endDate: p.endDate?p.endDate.slice(0,10):'', memberIds: (p.members||[]).filter(m=>m.role!=='admin').map(m=>m.id), clientIds: (p.clients||[]).map(c=>c.id) });
    setEditError('');
  };

  const saveEdit = async () => {
    if (!editForm.name) return setEditError('Name is required');
    try { await axios.put(`/projects/${editingId}`, editForm); setEditingId(null); fetchProjects(); }
    ...
    <div style={{ display:'flex', minHeight:'100vh', background:t.bg, fontFamily:"'DM Sans',sans-serif", transition:'background 0.3s' }}>
      ...
      <main style={{ flex:1, padding:'40px 48px', overflowY:'auto' }}>
        <div style={{ marginBottom:'36px' }}>
          ...
          <p style={{ color:t.textSec, fontSize:'14px', margin:0 }}>{isAdmin ? `${projects.length} project${projects.length!==1?'s':''} in your workspace` : 'Projects assigned to you'}</p>
        </div>
        ...
        {isAdmin && (
          <div style={{ background:t.cardBg, borderRadius:'16px', padding:'28px', border:`1px solid ${t.border}`, marginBottom:'32px', boxShadow: dark?'0 4px 24px rgba(0,0,0,0.3)':'0 2px 12px rgba(0,0,0,0.05)' }}>
            ...
            {error && <div style={{ background:dark?'rgba(239,68,68,0.1)':'#fef2f2', border:'1px solid rgba(239,68,68,0.3)', borderRadius:'10px', padding:'10px 14px', color:'#f87171', fontSize:'13px', marginBottom:'16px', display:'flex', alignItems:'center', gap:'8px' }}>⚠ {error}</div>}
            <FormBody f={form} setF={setForm} ms={mSearch} setMs={setMSearch} cs={cSearch} setCs={setCSearch} nonClients={nonClients} clients={clients} dark={dark} t={t} />
        ...
        {projects.length === 0 ? (
          <div style={{ background:t.cardBg, borderRadius:'16px', padding:'64px', border:`1px solid ${t.border}`, textAlign:'center' }}>
            ...
            <p style={{ color:t.textSec, fontSize:'15px', margin:0 }}>{isAdmin ? 'No projects yet. Create your first one above.' : 'No projects assigned to you yet.'}</p>
          </div>
        ) : (
                ...
                <div key={p.id} className="sp-card" style={{ background:t.cardBg, borderRadius:'16px', padding:'24px', border:`1px solid ${t.border}`, animationDelay:`${i*0.06}s`, boxShadow:dark?'0 4px 24px rgba(0,0,0,0.3)':'0 2px 12px rgba(0,0,0,0.05)' }}>
                  ...
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'14px' }}>
                    ...
                    <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end', gap:'5px' }}>
                      ...
                      {isOver && p.status !== 'completed' && <span style={{ padding:'2px 8px', borderRadius:'20px', fontSize:'10px', fontWeight:'600', background:'rgba(239,68,68,0.1)', color:'#f87171' }}>⚠ Overdue</span>}
                    </div>
                  </div>
        ...
        {editingId && (
          <div onClick={e => { if(e.target===e.currentTarget) setEditingId(null); }} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.65)', backdropFilter:'blur(5px)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center', padding:'20px' }}>
            <div className="sp-modal" style={{ background:t.cardBg, borderRadius:'20px', padding:'32px', width:'100%', maxWidth:'780px', maxHeight:'90vh', overflowY:'auto', border:`1px solid ${t.border}`, boxShadow:'0 24px 80px rgba(0,0,0,0.45)' }}>
              ...
              {editError && <div style={{ background:'rgba(239,68,68,0.1)', border:'1px solid rgba(239,68,68,0.3)', borderRadius:'10px', padding:'10px 14px', color:'#f87171', fontSize:'13px', marginBottom:'16px' }}>⚠ {editError}</div>}
              <FormBody f={editForm} setF={setEditForm} ms={emSearch} setMs={setEmSearch} cs={ecSearch} setCs={setEcSearch} nonClients={nonClients} clients={clients} dark={dark} t={t} />
