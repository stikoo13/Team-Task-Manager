...
export default function Projects() {
  ...
  const createProject = async () => {
    ...
    try {
      ...
      setForm(EMPTY); setMSearch(''); setCSearch(''); setError('');
      fetchProjects();
  ...
  const UserPicker = ({ list, selected, onToggle, empty, accentColor = '#6366f1', accentBg = 'rgba(99,102,241,0.1)' }) => (
  ...
  // ── Shared form body (used in create + edit modal) ────────────
  const FormBody = ({ f, setF, ms, setMs, cs, setCs, focusPrefix='' }) => (
    <>
      {/* Row 1 */}
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'14px', marginBottom:'14px' }}>
        {[{k:'name',label:'Project Name',ph:'e.g. Website Redesign'},{k:'description',label:'Description',ph:'Brief project description'}].map(fd => (
          <div key={fd.k}>
            ...
            <input placeholder={fd.ph} value={f[fd.k]}
              onFocus={() => setFocused(focusPrefix+fd.k)} onBlur={() => setFocused('')}
              onChange={e => setF({...f,[fd.k]:e.target.value})}
              style={inp(focusPrefix+fd.k)} />
          </div>
        ))}
      </div>
      {/* Row 2: status + dates */}
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:'14px', marginBottom:'20px' }}>
        <div>
          ...
          <select value={f.status} onChange={e => setF({...f,status:e.target.value})}
            onFocus={() => setFocused(focusPrefix+'status')} onBlur={() => setFocused('')}
            style={inp(focusPrefix+'status')}>
        ...
        {[{k:'startDate',label:'Start Date'},{k:'endDate',label:'Deadline'}].map(fd => (
          <div key={fd.k}>
            ...
            <input type="date" value={f[fd.k]}
              onFocus={() => setFocused(focusPrefix+fd.k)} onBlur={() => setFocused('')}
              onChange={e => setF({...f,[fd.k]:e.target.value})}
              style={inp(focusPrefix+fd.k, {colorScheme: dark?'dark':'light'})} />
          </div>
        ))}
      </div>
      {/* Row 3: members + clients */}
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'20px' }}>
        <div>
          <label style={lbl}>Assign Members ({f.memberIds.length} selected)</label>
          <input placeholder="Search members..." value={ms} onChange={e => setMs(e.target.value)}
            onFocus={() => setFocused(focusPrefix+'ms')} onBlur={() => setFocused('')}
            style={{...inp(focusPrefix+'ms'), marginBottom:'8px'}} />
          <UserPicker list={flt(nonClients, ms)} selected={f.memberIds}
            onToggle={id => toggle('memberIds', id, setF)} empty="No members found" />
        </div>
        <div>
          <label style={{...lbl}}>
            <span style={{color:'#34d399'}}>◉ </span>
            Assign Clients ({f.clientIds.length} selected)
          </label>
          <input placeholder="Search clients..." value={cs} onChange={e => setCs(e.target.value)}
            onFocus={() => setFocused(focusPrefix+'cs')} onBlur={() => setFocused('')}
            style={{...inp(focusPrefix+'cs'), marginBottom:'8px'}} />
          <UserPicker list={flt(clients, cs)} selected={f.clientIds}
            onToggle={id => toggle('clientIds', id, setF)} empty="No clients found"
            accentColor="#34d399" accentBg="rgba(52,211,153,0.1)" />
        </div>
      </div>
    </>
  );
      ...
      <main style={{ flex:1, padding:'40px 48px', overflowY:'auto' }}>
        ...
        {isAdmin && (
          <div style={{ background:t.cardBg, borderRadius:'16px', padding:'28px', border:`1px solid ${t.border}`, marginBottom:'32px', boxShadow: dark?'0 4px 24px rgba(0,0,0,0.3)':'0 2px 12px rgba(0,0,0,0.05)' }}>
            ...
            <FormBody f={form} setF={setForm} ms={mSearch} setMs={setMSearch} cs={cSearch} setCs={setCSearch} focusPrefix="c_" />
            <button onClick={createProject} style={{ marginTop:'20px', padding:'11px 28px', background:'linear-gradient(135deg,#6366f1,#8b5cf6)', color:'white', border:'none', borderRadius:'10px', fontSize:'14px', fontWeight:'600', cursor:'pointer', fontFamily:"'DM Sans',sans-serif", boxShadow:'0 4px 14px rgba(99,102,241,0.35)', transition:'all 0.2s' }}>
              + Create Project
            </button>
          </div>
        )}
        ...
        {editingId && (
          <div
            ...
            <div className="sp-modal" style={{ background:t.cardBg, borderRadius:'20px', padding:'32px', width:'100%', maxWidth:'780px', maxHeight:'90vh', overflowY:'auto', border:`1px solid ${t.border}`, boxShadow:'0 24px 80px rgba(0,0,0,0.45)' }}>
              ...
              <FormBody f={editForm} setF={setEditForm} ms={emSearch} setMs={setEmSearch} cs={ecSearch} setCs={setEcSearch} focusPrefix="e_" />
